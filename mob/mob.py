...
class CompanionCore:
    ...
    def refresh(self):
        ...
        self.snapshot = {
            ...
            "alerts": alerts,
            "memory": memory,
            # Full bonus scoring arrives in its own roadmap phase.
            "bonus_status": "available_later" if not remaining else "locked",
        }
    ...
    def opening_message(self):
        ...
        if not recommendation:
            return f"Welcome back. {memory_line} {self.mode} mode is active and all five core tasks are logged — clean finish is available."
    ...
    def task_saved_message(self, saved_tasks):
        ...
        if not rec:
            return f"✅ {names} saved • +{earned} XP. All five core tasks are logged. Clean finish is available."
    ...
    def task_logged_message(self, canonical, minutes):
        task = get_task(canonical)
        ...
        if not rec:
            return base + " All five core tasks are logged. Clean finish is available."
...
def handle_command(text, state):
    ...
    if canonical and any(w in low for w in ["done", "did", "finished", "complete", "completed", "logged", "log"]):
        ...
        return get_companion_core(state).task_logged_message(canonical, mins)

    return "Try: next, goal, goals, why, memory, tasks, history, mode, quote, log Spanish 20, or end day."
...
def quick_log_mission(state, recommendation, minutes):
    ...
    message = refreshed_core.task_logged_message(canonical, minutes)
    ...
    st.session_state["quick_mission_flash"] = (
        ...
        f"logged • +{task_xp(canonical, minutes)} XP"
    )
    st.rerun()
...
def render_header(state, animate_mission=False):
    ...
    if recommendation:
        ...
        focus_reward = task_xp(focus["canonical"], focus_minutes)
        st.markdown(
            f"""
            <div class='focusCard {'animateMission' if animate_mission else ''}'>
              ...
              <div class='focusMain'>
                <div>
                  <div class='focusTask'>🎯 {focus['display']}</div>
                  <div class='focusTime'>{focus_minutes} minutes</div>
                </div>
